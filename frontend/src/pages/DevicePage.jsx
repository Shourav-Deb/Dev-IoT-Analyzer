...
export default function DevicePage() {
  ...
  async function load() {
    const url = id
      ? `/.netlify/functions/getDeviceData?device_id=${id}`
      : "/.netlify/functions/getDeviceData";
  ...
  useEffect(() => {
    ...
    const idt = setInterval(
      load,
      parseInt(import.meta.env.VITE_POLL_INTERVAL || 5000)
    );
  ...
  return (
    <div>
      ...
      <div className="live-grid">
        <div>
          Power: <strong>{parsed.watt ?? "-"} W</strong>
        </div>
        <div>
          Voltage: <strong>{parsed.voltage ?? "-"} V</strong>
        </div>
        <div>
          Current: <strong>{parsed.current ?? "-"} A</strong>
        </div>
        <div>
          kWh: <strong>{parsed.total_kwh ?? 0} kWh</strong>
        </div>
      </div>
